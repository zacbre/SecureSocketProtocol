...
namespace SecureSocketProtocol2.Hashers
{
    public class MurmurHash2UInt32Hack : ISeededHashAlgorithm
    {
        ...
        public UInt32 Hash(Byte[] data, int length, int offset)
        {
            if (length == 0)
                return 0;
            ...
            while (length >= 4)
            {
                UInt32 k = hackArray[(currentIndex++) + offset];
                k *= m;
                k ^= k >> r;
                k *= m;
            ...
            {
                case 3:
                    h ^= (UInt16)(data[(currentIndex++) + offset] | data[(currentIndex++) + offset] << 8);
                    h ^= (UInt32)data[currentIndex + offset] << 16;
                    h *= m;
                    break;
                case 2:
                    h ^= (UInt16)(data[(currentIndex++) + offset] | data[currentIndex + offset] << 8);
